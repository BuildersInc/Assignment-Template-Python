...
def get_parser():
    ...
    new_parser = argparse.ArgumentParser(
        description="<ScriptDescription>", formatter_class=argparse.RawTextHelpFormatter
    )

    debugging_utils = new_parser.add_argument_group(
        "Debugging Utils", "Debugging flags"
    )

    debugging_utils.add_argument(
        "-v",
        "--verbosity",
        required=False,
        action="count",
...
def setup_logger(args) -> None:
    ...
    level = logging.DEBUG - args.verbosity + 1 if args.verbosity else logging.INFO
